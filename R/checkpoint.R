################################################################################
##  checkpoint.R — Ejecución recuperable mediante checkpoints
##  --------------------------------------------------------------------------
##  Este script define utilidades para guardar y recuperar resultados
##  intermedios del pipeline. Cada etapa computacionalmente costosa puede
##  envolverse con cache_step(), que almacena su resultado en formato RDS y lo
...
#'
#' @param name  identificador del paso (por ejemplo "tad_reduced", "crm_intratad").
...
clear_checkpoints <- function(chr, names = NULL) {
  ...
  else .msg("No se encontraron checkpoints que borrar para ", chr, ".")
  invisible(fs)
}
